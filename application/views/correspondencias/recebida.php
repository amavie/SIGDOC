                    ...
                    if ($r->estadosRec==0) {
                        ...
                        echo "<td>";
                            if($this->permission->checkPermission($this->session->userdata('permissao'),'pCorrespondencia')){
                                 echo '<a href="#modal-recebida" style="margin-right: 1%" role="button" data-toggle="modal" correspondenciaRec="'.$r->correspondencias.'" idCorrespondenciasRec="'.$r->correspondencias_id.'" tipo_pro="'.$r->tipo_pro.'" class="btn btn-success tip-top" title="Correspondência já recebida"><i class="icon-envelope-alt icon-white"></i></a>';
                            }
                        echo  '</td>';                        
                    }
                    if ($r->estadosRec==1) {
                        ...
                        echo "<td>";
                             if($this->permission->checkPermission($this->session->userdata('permissao'),'pCorrespondencia')){
                                 echo '<a href="#" style="margin-right: 1%" role="button" data-toggle="modal" correspondenciaRec="'.$r->correspondencias.'" idCorrespondenciasRec="'.$r->correspondencias_id.'" tipo_pro="'.$r->tipo_pro.'" class="btn btn-secondary tip-top" title="Correspondência recebida"><i class="fa fa-envelope-open-o icon-white"></i></a>';
                            }
                        echo  '</td>';                        
                    }
                     ...
                     echo '<td>';
                      if($this->permission->checkPermission($this->session->userdata('permissao'),'detCorrespondencia')){
                            echo '<a href="'.base_url().'index.php/correspondencias/visualizar_dados/'.$r->correspondencias_id.'"  class="btn btn-info tip-top" style="margin-right: 1%" title="Ver mais detalhes"><i class="icon-eye-open"></i></a>';                     
                        } 
                    echo  '</td>';
...
<!-- Modal Recebida-->
<div id="modal-recebida" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <form action="<?php echo base_url() ?>index.php/correspondencias/confirmacao" id="parecer" method="post" >
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h5 id="myModalLabel">Confirmação da recepçao</h5>
  </div>

  <label style="padding-top: 20px; margin-left: 10px;" for="observacao" class="control-label">Dados da Correspondência:</label>
                     
  
    <input style="border: 0; margin-left: 10px;" type="text"  disabled id="correspondenciaRec">

  <div  class="control-group" >
                
    <input type="hidden" id="idCorrespondenciasRec" name="correspondencias_id" /><!-- id da correspondencia-->
    <input type="hidden" id="usuarios"  name="usuarios" value="<?php  echo $this->session->userdata('id'); ?>"  /> 
  ...
  <div class="modal-footer">
    <button class="btn" data-dismiss="modal" aria-hidden="true">Cancelar</button>
    <button class="btn btn-danger">Recebida</button>
  </div>
  </form>
</div>

<!-- Modal Receber 2-->
<div id="modal-gggg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <form action="<?php echo base_url() ?>index.php/correspondencias/parecer" id="parecer" method="post" >
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h5 id="myModalLabel">Parecer Correspondência</h5>
...
$(document).ready(function(){
   ...
   $(document).on('click', 'a', function(event) {
        ...
        var idCorrespondenciasRec = $(this).attr('idCorrespondenciasRec');
        $('#idCorrespondenciasRec').val(idCorrespondenciasRec);
        ...
        var correspondenciaRec = $(this).attr('correspondenciaRec');
        $('#correspondenciaRec').val(correspondenciaRec);
