...
class Correspondencias extends CI_Controller {
    ...
    if(!$this->permission->checkPermission($this->session->userdata('permissao'),'vCorrespondencia')){
        ...
        $this->load->view('correspondencias/correspondencias');
...
public function gabinete(){
        ...
        $this->data['resultsDespacho'] = $this->Correspondencias_model->getDespachoGabinete();
        $this->data['resultsPar'] = $this->Correspondencias_model->getParecer($this->uri->segment(3)); 
        }
        else{

           $this->data['resultsDespacho'] = $this->Correspondencias_model->search_recebida_tra($pesquisa, $de, $ate);
            $this->data['resultsPar'] = $this->Correspondencias_model->search_recebida_par($pesquisa, $de, $ate);
        ...
        $this->data['view'] = 'relatorios/gabinete';
        $this->load->view('tema/topo',$this->data);
    }
     ...
     public function confirmacao(){
        ...
        $data = array( 'estadoRec' => 1); 
        $id=$this->input->post('correspondencias_id');                
        $results = $this->Correspondencias_model->edit('correspondencias', $data, 'id', $id);
if ($results) {
   $this->session->set_flashdata('success','Recepção da correspondência feita com sucesso!');
        redirect(base_url() . 'index.php/correspondencias/recebida');
}
